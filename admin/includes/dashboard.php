...
class HippoDash {
	...
	public function check_validation_init( $post_id = NULL ){
		if ($post_id === NULL ){
			...
			if ($test_posts){
				foreach ((array)$test_posts as $post) {
	...
	public function meta_title( $pid, $length = false ){
		if($pid){
			...
			if($length){
				if( strlen($meta_title) > get_option('char_limit_title') ) { 
					return 1;
				} else {
					return 0;
				}
	...
	public function meta_desc( $pid, $length = false ){
		if($pid){
			...
			if($length){
				if(strlen( $meta_desc) > get_option('char_limit_alt') + 1 ) { 
					return 1;
				} else {
					return 0;
				}
	...
	public function img_title( $pid, $length = false ){
		if($pid){
			...
			if($length){
				if( strlen($img_title) > get_option('img_limit_title') ) { 
					return 1;
				} else {
					return 0;
				}
	...
	public function img_alt( $pid, $length = false ){
		if($pid){
			...
			if($length){
				if( strlen($image_alt) > get_option('img_limit_alt') ) { 
					return 1;
				} else {
					return 0;
				}
